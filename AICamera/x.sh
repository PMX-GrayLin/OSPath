...
if [ "$1" = "aic" ] ; then
	...
	if [ "$2" = "jobs" ] ; then
		...
		if [ "$3" = "usb" ] ; then
			...
			if [ "$4" = "tee" ] ; then
				# NG
				cmd="gst-launch-1.0, gst-launch-1.0 v4l2src device=${VIDEO_DEV[0]} ! tee name=t t. ! videoconvert ! video/x-raw,format=YUY2,width=1920,height=1080 ! queue ! fpsdisplaysink video-sink=waylandsink sync=false t. ! queue ! v4l2h264enc extra-controls=cid,video_gop_size=30 capture-io-mode=dmabuf ! h264parse config-interval=1 ! rtspclientsink location=rtsp://localhost:8554/mystream"
			elif [ "$4" = "dp" ] ; then
