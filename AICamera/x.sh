	...
	if [ "$2" = "jobs" ] ; then
		...
		if [ "$third_arg" = "usb" ] ; then
			...
			if [ "$4" = "tee" ] ; then
				...
				cmd='gst-launch-1.0 aravissrc camera-name=id1 ! videoconvert ! video/x-raw,format=NV12,width=1536,height=1024 ! fpsdisplaysink video-sink=waylandsink sync=false text-overlay=true'
			elif [ "$4" = "dp2" ] ; then
				cmd='gst-launch-1.0 aravissrc camera-name=id1 ! videoconvert ! video/x-raw,format=NV12,width=3072,height=2048 ! fpsdisplaysink video-sink=waylandsink sync=false text-overlay=true'
			else
